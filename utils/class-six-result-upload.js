...
const storage = multer.diskStorage({
  ...
  filename: function(req, file, cb) {
    ...
    const finalName = `Class_Six_Result_File_${Date.now()}${extension}`;
    cb(null, finalName);
  },
});
    ...
    try {
      ...
      for (let row of data) {
        ...
        await classSixResultModel.create({
          ...
          class_six_result: `/temp/result-files/${req.file.filename}`
        });
      }

      req.flash("success", "Class six result successfully published.");
