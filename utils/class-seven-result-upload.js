...
const storage = multer.diskStorage({
  ...
  filename: function(req, file, cb) {
    ...
    const finalName = `Class_Seven_Result_File_${Date.now()}${extension}`;
    cb(null, finalName);
  },
});
    ...
    try {
      ...
      for (let row of data) {
        await classSevenResultModel.create({
          ...
          class_seven_result: `/temp/result-files/${req.file.filename}`
        });
      }

      req.flash("success", "Class seven result successfully published.");
